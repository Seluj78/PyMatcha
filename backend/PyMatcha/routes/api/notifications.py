...
def get_all_notifications():
    logging.info(f"Returning all notifications for user {current_user.id}")
...
def get_unread_notifications():
    logging.info(f"Returning unread notifications for user {current_user.id}")
...
def mark_notification_as_read(n_id):
    ...
    notif.is_seen = True
    notif.save()
    logging.debug(f"Marking notification {n_id} as read")

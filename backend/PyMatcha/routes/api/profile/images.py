    ...
    if file:
        if is_primary:
            if not Image.get_multi(user_id=current_user.id, is_primary=True):
                ...
                logging.info(f"Added primary image for {current_user.id}")
            ...
            logging.info(f"Added image for {current_user.id}")
            return SuccessOutput("image", link)
    else:
...
def delete_image_profile(image_id):
    ...
    image.delete()
    logging.info(f"Deleted image for {current_user.id}")
...
def change_main_image(image_id):
    ...
    image.save()
    logging.info(f"Modified profile picture for {current_user.id}")

...
def block_profile(uid):
    ...
    if current_user.id == u.id:
        logging.info(f"User {current_user.id} tried to block itself")
    ...
    if not Block.get_multi(blocker_id=current_user.id, blocked_id=u.id):
        ...
        logging.info(f"Block successfull from {current_user.id} to {u.id}")
        return Success(f"Successfully blocked {u.id}.")
    else:
        logging.info(f"{current_user.id} already blocked {u.id}")
...
def unblock_profile(uid):
    ...
    if not block:
        logging.info(f"{current_user.id} tried to unblock {u.id} but never blocked it first")
        raise BadRequestError("You didn't block this user.")
    else:
        block.delete()
        logging.info(f"Unblock successfull from {current_user.id} to {u.id}")

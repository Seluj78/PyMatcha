    ...
    <div class="relative overflow-hidden bg-transparent rounded-md w-full" style="padding-bottom: 70%">
      ...
      <div class="absolute top-0 left-0 z-20 m-5">
        ...
        <div v-if="showImageSettings" class="z-30 mt-4">
          <h1 class="text-red-500 font-bold text-center bg-white-matcha px-4 py-2 cursor-pointer rounded-md" v-on:click="deleteImage(image.id)">Delete</h1>
          <h1 class="text-green-500 font-bold text-center bg-white-matcha px-4 py-2 mt-2 cursor-pointer rounded-md" v-if="!image.is_primary" v-on:click="makePrimaryImage(image.id)">Make primary</h1>
        </div>
      </div>
    </div>
  </div>
